int rowclash(int **box,int i,int j,int num){
    for(int col=0; col<N;col++){
        if(col!=j && box[i][col]==num) return 1;
    }
    return 0;
}
int columnclash(int **box,int i,int j,int num){
    for(int row=0; row<N;row++){
        if(row!=i && box[row][j]==num) return 1;
    }
    return 0;
}
int boxclash(int **box,int i,int j,int num){
...
int clash(int **box,int i,int j,int num){
...
int validinput(int **box,int i,int j,int num){
